...
function getCategory1List(){
	...
	var html = utils.httpGetAsString("https://m.huya.com",headers);
//print("html = "+html);	
	var doc = utils.newHTMLDocument(html);	
	var ea = doc.getBody().querySelectorAll(">body div#root  nav  div.nav-links a");
 //print("ea.length = "+ea.length);
	var items = [];
	for(e of ea){
		var href = e.getAttribute("href");
		var p;
		if( (p=href.indexOf("/g/"))<0 ){
			continue;
		}
		var tit = e.getAllNodeValue().trim();
		items.push({title: tit, items:"@huyalive-list:"+href.substring(p+1)});
	}
	return items;
}
